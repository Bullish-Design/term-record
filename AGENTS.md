...
TermRecord is a NixOS flake that provides automatic terminal recording linked to atuin shell history. This guide provides implementation instructions for Claude Code.
...
#### 1.1 `src/termrecord/models/config.py`

```python
# src/termrecord/models/config.py
```
...
#### 1.2 `src/termrecord/models/metadata.py`

```python
# src/termrecord/models/metadata.py
```
...
#### 1.3 `src/termrecord/models/index.py`

```python
# src/termrecord/models/index.py
```
...
- `check_path_cli()`: Entry point for `termrecord-check-path` command

Path checking logic:
1. Walk from `directory` toward `/` looking for `.termrecord.toml`
    ...
    if [[ -n "$BUFFER" ]] && _termrecord_should_record; then
        ...
        case "${${(z)BUFFER}[1]}" in
            cd|pushd|popd|export|unset|source|.|eval|exec|...)
                ...
                _termrecord_setup_recording "$BUFFER"
                _TERMRECORD_SKIP_WRAP=1
                ;;
            *)
                # Wrap with asciinema rec
                if _termrecord_setup_recording "$BUFFER"; then
                    BUFFER="asciinema rec --quiet --overwrite -c ${(q)BUFFER} ${(q)_TERMRECORD_CAST_FILE}"
                fi
                ;;
        esac
    fi
...
3. `_termrecord_setup_recording` creates `.pending` file for `ls -la`
...
For these, we create `.pending` but set `_TERMRECORD_SKIP_WRAP=1`. In `precmd`, if skip flag is set, delete `.pending` (no `.cast` was created).

### Phase 4: Watcher Service

#### 4.1 `src/termrecord/watcher/main.py`
...
#### 4.2 `src/termrecord/watcher/file_watcher.py`
...
#### 4.3 `src/termrecord/watcher/indexer.py`
...
#### 4.4 `src/termrecord/watcher/exporter.py`
...
#### 4.5 `src/termrecord/watcher/cleanup.py`
...
#### 4.6 `src/termrecord/watcher/server.py`
...
#### 5.1 `src/termrecord/cli/main.py`
...
#### 5.2 `src/termrecord/cli/client.py`
...
[project.scripts]
termrecord = "termrecord.cli.main:main"
termrecord-watcher = "termrecord.watcher.main:main"
termrecord-check-path = "termrecord.config:check_path_cli"
...
- Include scripts/ in `$out/share/termrecord/`
...
Key features:
- Generate `~/.config/termrecord/config.toml` via `xdg.configFile`
...
2. **Second line**: `# src/termrecord/path/to/file.py` (filepath comment)
...
class Config(BaseModel):
    storage_dir: Path = Path("~/.local/share/termrecord").expanduser()
...
```bash
time (for i in {1..100}; do _termrecord_should_record; done)
```
...
The atuin ID isn't available until atuin's precmd runs. Our precmd must run after atuin's. In practice, this means sourcing termrecord hooks AFTER atuin init in shell rc.
...
| Variable | Purpose |
|----------|---------|
| `TERMRECORD_ENABLED` | Override: `0` disables, `1` enables |
| `TERMRECORD_STORAGE` | Override storage directory |
| `TERMRECORD_CONFIG` | Override config file path |
| `TERMRECORD_CAST_FILE` | Set by preexec, path for recording |
| `TERMRECORD_RECORDING_ID` | Set by preexec, current recording ID |
...
1. `src/termrecord/models/config.py` - Config models
2. `src/termrecord/models/metadata.py` - Metadata models  
3. `src/termrecord/config.py` - Config loading
4. `scripts/hooks.zsh` - ZLE widget + precmd hook
5. `src/termrecord/watcher/indexer.py` - SQLite layer
6. `src/termrecord/watcher/file_watcher.py` - inotify
7. `src/termrecord/watcher/main.py` - Minimal watcher
8. `src/termrecord/cli/main.py` - Basic CLI
